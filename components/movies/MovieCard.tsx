...
export function MovieCard({ movie, showRating = true, variant = "default" }: MovieCardProps) {
  ...
  if (variant === "featured") {
    return (
      <Link href={`/movies/${movie.id}`}>
        <article 
          className="group relative h-[400px] rounded-2xl overflow-hidden bg-[#1A2236]"
          ...
          {showRating && avgRating && (
            <div className="absolute top-4 right-4 z-10">
              <div className="bg-black/60 backdrop-blur-sm rounded-lg px-3 py-1.5 flex items-center gap-1.5">
                <span className="text-[#FF8400] text-lg">★</span>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6">
            <div className="flex flex-wrap gap-2 mb-3">
              {movie.genres?.slice(0, 3).map((mg) => (
                <span key={mg.genre.slug} className="px-2 py-0.5 bg-[#FF8400]/80 text-white text-xs font-mono font-medium rounded">
                  {mg.genre.name}
                </span>
              ))}
            </div>
            
            <h3 className="font-oswald text-2xl font-bold text-white mb-2 group-hover:text-[#FF8400] transition-colors">
              {movie.title}
            </h3>
            
            <div className="flex items-center gap-3 font-mono text-[13px] text-[#8B95A8]">
              {year && <span>{year}</span>}
  ...
  return (
    <Link href={`/movies/${movie.id}`}>
      <article 
        className="group relative bg-[#1A2236] rounded-2xl overflow-hidden hover:ring-1 hover:ring-[#FF8400]/30 transition-all duration-500 h-full flex flex-col hover:shadow-2xl hover:shadow-[#FF8400]/5 hover:-translate-y-1"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <div className="relative aspect-[2/3] overflow-hidden flex-shrink-0">
          ...
          <div className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${isHovered ? "opacity-100 scale-100" : "opacity-0 scale-75"}`}>
            <div className="w-14 h-14 bg-[#FF8400] rounded-full flex items-center justify-center shadow-lg shadow-[#FF8400]/50 hover:bg-[#FF9F2E] transition-colors">
          ...
          <div className={`absolute bottom-0 left-0 right-0 p-3 transition-all duration-300 ${isHovered ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            <div className="flex items-center justify-between font-mono text-[11px] text-white/80">
              {movie.runtime && <span>⏱️ {movie.runtime} {t("minutes")}</span>}
              {year && <span>📅 {year}</span>}
            </div>
          </div>
        </div>
        ...
        <div className="p-3 flex flex-col flex-grow">
          <h3 className="font-mono text-[13px] font-semibold text-white group-hover:text-[#FF8400] transition-colors line-clamp-2 min-h-[2.5rem]">
            {movie.title}
          </h3>
          
          {movie.originalTitle && movie.originalTitle !== movie.title && (
            <p className="font-mono text-[11px] text-[#5A6478] mt-0.5 line-clamp-1">
              {movie.originalTitle}
            </p>
          )}
            ...
            {movie.genres && movie.genres.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {movie.genres.slice(0, 2).map((mg) => (
                  <span key={mg.genre.slug} className="font-mono text-[10px] text-[#8B95A8] bg-[#2A3550] px-2 py-0.5 rounded">
                    {mg.genre.name}
                  </span>
                ))}
              </div>
