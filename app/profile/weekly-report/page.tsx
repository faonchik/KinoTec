...
export default function WeeklyReportPage() {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert("Отчёт отправлен на ваш email!");
      } else {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-16">
          ...
          <p className="text-slate-400 mb-6">Начните смотреть фильмы, чтобы получить отчёт</p>
          <Link href="/movies">
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Еженедельный отчёт</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
          ...
          <div className="text-slate-400 text-sm">Фильмов</div>
        </div>
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
          <div className="text-3xl font-bold text-white mb-1">{report.watchedSeries}</div>
          <div className="text-slate-400 text-sm">Сериалов</div>
        </div>
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
          <div className="text-3xl font-bold text-white mb-1">{report.hours}</div>
          <div className="text-slate-400 text-sm">Часов просмотра</div>
        </div>
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-slate-400">Оставлено оценок</span>
              <span className="text-white font-semibold">{report.ratings}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Написано отзывов</span>
        ...
        {report.topGenres.length > 0 && (
          <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-white mb-4">Любимые жанры</h3>
      ...
      {report.movies.length > 0 && (
        <section className="mb-8">
          <h2 className="text-2xl font-bold text-white mb-4">Просмотренные фильмы</h2>
      ...
      {report.series.length > 0 && (
        <section>
          <h2 className="text-2xl font-bold text-white mb-4">Просмотренные сериалы</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
